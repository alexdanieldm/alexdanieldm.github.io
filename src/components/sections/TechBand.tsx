 ...
 * banner.
 *
 * The names are the same in every language, so they are not content.
 *
 * Sizes are measured, not chosen. One size for all twelve does not give twelve
 * marks that look the same size, because each one sits differently inside its
 * own viewBox: at a 46px box the drawn ink ranged from 29px to 46px across the
 * row, and the padded ones read as small. Each size below is 46 divided by that
 * mark's measured ink ratio, so every mark's longest side draws at the same
 * 46px. Re-measure rather than nudge by eye if a mark is ever replaced.
 */
const STACK = [
...
export function TechBand({ content }: { content: HomeContent['tech'] }) {
  return (
    <section className={styles.band} id="tech" aria-labelledby="tech-title">
      <SectionHeading id="tech-title" kicker={content.kicker} tone="on-accent">
        {content.heading}
      </SectionHeading>
